...
// them, so running `tyche generate` is never required — it just removes
// reflection from the hot path. Types declared in package main are keyed to
// match Go's runtime reflection, so single-file main.go apps work too.
package servergen
